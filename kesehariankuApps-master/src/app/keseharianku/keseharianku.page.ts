...
@Component({
  selector: 'app-keseharianku',
  templateUrl: './keseharianku.page.html',
  styleUrls: ['./keseharianku.page.scss'],
})
export class kesehariankuPage implements OnInit {
  nama: any;
  ...
  ionViewDidEnter() {
    ...
    this.page = 0;
    this.perPage = 10;
    this.getkeseharianku();
  }

  paginateArray() {
    this.page++;
    return this.keseharianku.filter(
      (x) =>
  ...
  getkeseharianku() {
    this._apiService.tampil('tampilkeseharianku.php').subscribe({
      next: (res: any) => {
        console.log('sukses', res);
        this.keseharianku = res;
        ...
        buttons: [
          ...
          },
          {
            ...
            handler: (data: any) => {
              //jika tekan yakin
              this._apiService
                .hapus(id, '/hapuskeseharianku.php?id=')
                .subscribe({
                  next: (res: any) => {
                    console.log('sukses', res);
                    this.page = 0;
